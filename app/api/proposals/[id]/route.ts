...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  // Passer un devis à « envoyé », c'est l'envoyer au client : soumis à
  // l'autorisation `envoyer_devis`. Un membre qui ne l'a pas prépare le devis,
  // le titulaire l'envoie. Le titulaire a toujours le droit.
  if (modifs.status === "sent") {
...
export async function DELETE(_req: NextRequest, { params }: Params) {
  ...
  // Supprimer un devis est réservé au titulaire.
  ...
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  if (!supprime) return NextResponse.json({ error: "Devis introuvable" }, { status: 404 });
